<template>
	<view class="login">
		...
		<view class="vision" @tap="check('manual')" :style="{top: positionTop + 'px'}">
			<text>当前版本：</text>
...
<script>
	...
	import {login, getUserInfo, checkVersion} from '@/common/api/login/api.js'
	import {validatenull} from '@/common/util/validate.js'
	import { mapGetters } from 'vuex';
	import tuiModal from "@/components/modal"
	import crypto from '@/common/crypto-js/crypto-js.js'
	import sha256 from '@/common/crypto-js/sha256.js'
	export default {
		data() {
			return {
				modal: false,
...
      	that.currenVersion = inf.versionCode
				that.positionTop = uni.getSystemInfoSync().windowHeight - 28;
				that.check()
...
      })
		...
		methods: {
			judgeToken () {
				if (!validatenull(uni.getStorageSync('userInfo'))) {
					this.userName = uni.getStorageSync('userInfo').loginMsg.username
					this.passWord = uni.getStorageSync('userInfo').loginMsg.password
				}
				let loginMsg = {
				...
				if (uni.getStorageSync('access_token')) {
					getUserInfo().then(res => {
						if (res.status) {
							uni.setStorageSync('userInfo', {loginMsg, ...res.data})
			...
			handleClick (e) {
				...
				if (index === 1) {
					...
					/* uni.downloadFile({
					    url: `http://10.1.16.50/usr/local/cargoApp/version/${this.currenVersion}`,
					    success: (res) => {
			...
			check (flag) {
				checkVersion().then(res => {
					if (res.status) {
						/* let url = `http://10.1.16.50:10001/menu/downloadFile?fileName=cargo-${res.message}.apk`
						plus.runtime.openURL(url) */
						// #ifdef APP-PLUS
						if (Number(this.currenVersion) !== Number(res.message)) {
							this.newCurrenVersion = res.message
							this.modal = true
						} else {
							if (flag === 'manual') {
								this.$message('当前为最新版本！')
							}
							this.judgeToken()	
						}
						// #endif
		    ...
		    startLogin(){
				...
				if (this.userName.length == '' || this.passWord.length == "") {
				     uni.showToast({
				        icon: 'none',
								position: 'bottom',
				        title: this.passWord.length == '' ? '密码不能为空' : '用户名不能为空'
				    });
				    return;
				}
				this.isRotate=true
				...
				let data = {
					username: this.userName,
					password: encodeURIComponent(crypto.enc.Base64.stringify(sha256(this.passWord))),
					grant_type: 'password'
				}
				login(data).then(res => {
					console.log(123, res)
